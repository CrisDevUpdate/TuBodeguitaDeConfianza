...
export function initRealtimeSyncProtocol(onSyncCallback) {
  ...
  const handleRevalidation = async () => {
    try {
      if (window.InventoryApp?.Firebase && typeof window.InventoryApp.Firebase.sincronizarTodoDesdeNube === 'function') {
        await window.InventoryApp.Firebase.sincronizarTodoDesdeNube();
      }
