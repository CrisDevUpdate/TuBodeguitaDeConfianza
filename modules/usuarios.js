...
function configurarVistasPorRol(usuario) {
    ...
    if (esAdmin) {
        ...
        // Si existen abonos o pagos pendientes de conciliación en PagosPorVerificar, avisar de inmediato al Administrador
        const abonosPendientes = (AppState.abonos || []).filter(a => a.estado === 'PENDIENTE_CONFIRMACION');
        const pagosVerifPend = (AppState.pagosPorVerificar || []).filter(p => !p.estado || p.estado === 'PENDIENTE_VERIFICACION' || p.estado === 'PENDIENTE_CONFIRMACION' || p.estado === 'PENDIENTE' || p.estado === 'POR_VERIFICAR');
        const totalPendientes = abonosPendientes.length + pagosVerifPend.length;

        if (totalPendientes > 0) {
            setTimeout(() => {
                if (typeof reproducirSonidoNotificacion === 'function') {
                    reproducirSonidoNotificacion();
                } else if (window.reproducirSonidoNotificacion) {
